...
F5R2.prototype = {
    ...
    create: function (config) {
        ...
        while(creater.creater) {
            creater = creater.creater;
        }
        creater.servers.lastport = parseInt(config.port);
        var has = false;
        creater.servers.list = creater.servers.list.map(function (server) {
        ...
        if (!has) {
            creater.servers.list.push({
                status: my.status,
                config: config
            })
        }
        // TODO: servers changed, emit message to frontend
        creater.writeHistory();
    },
    ...
    writeHistory: function () {
        ...
        _.ensureExists(path.join(profile, 'f5r2'));
        var historyFile = path.join(profile, 'f5r2', filename);
        var history = this.servers.list.map(function (server) {
            return {
                config: {
        ...
        fs.writeFile(historyFile, JSON.stringify(history), function (err) {
            ...
            console.log('Save history:', historyFile, history);
        });
    },
    ...
    readHistory: function () {
        ...
        var filename = 'history.json';
        var historyFile = path.join(profile, 'f5r2', filename);
        if (!_.existsSync(historyFile)) {
            return [];
        }
        var history = JSON.parse(fs.readFileSync(historyFile)) || [];
